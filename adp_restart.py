...
# 查询当前脚本是否运行
def running(etlJob, script, txdate, system):
    script = "{etlJob} {script} {txdate} {system}".format(etlJob=etlJob,
                                                          script=script,
                                                          txdate=txdate,
                                                          system=system)
    process = os.popen("ps -A | grep {script}".format(script=script))
...
def getDetilInfo():
    ...
    result = coon.execSql(sql)
    if coon.isError() or result is None:
...
def updateDeStatus(id, status):
    sql = "UPDATE %s.ETL_JOB_RESTART_DETAIL T1" \
          "   SET JOB_STATUS = '%s', " \
          "       LOG_PATH = '%s'" \
          " WHERE T1.ID = '%s'" % (_AOTOETLDB, status, _LOGFILE, id)
    result = coon.execSql(sql)
    if coon.isError() or int(result) <> 1:
